...
void DrawTextOnScreen(const char* text, int x, int y, D3DCOLOR color) {
	...
	if (font) {
		font->DrawTextA(nullptr, text, -1, &rect, DT_CENTER | DT_NOCLIP, D3DCOLOR_ARGB(255, 0, 0, 0));
		SetRect(&rect, x, y, x, y);
		font->DrawTextA(nullptr, text, -1, &rect, DT_CENTER | DT_NOCLIP, color);
	}
}
...
void DrawESPBox3D(const Vec3& head, const Vec3& feet, int thickness, float width, D3DCOLOR color) // without rotation angle
{
	...
	Vec2 s[8];
	for (int i = 0; i < 8; i++)
		if (worldToScreen(p[i], s[i], viewMatrix, windowWidth, windowHeight)) {
			return;
		}
